<template>
  <container>
    <!-- <h1>分析报告</h1> -->
    <el-header style="height:50px; font-size: 22px" > 2020年4月云南省劳动力流向视图</el-header>
...
export default {
  ...
  mounted() {
    ...
    scene.on('loaded', () => {
      // + year_month
      fetch(conf.dev_base_url + '/api/labor_flow?year_month=202004' )
          .then(res => res.json())
          .then(data => {
            data=data.data
  ...
  methods: {
    empPopulationFlowData:(year_month)=>{
      ...
      window.fetch('http://127.0.0.1:5000/api/labor_flow_data?year_month=202004')
